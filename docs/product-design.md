...
| ASR 能力 | 中文实时语音识别应优先选成熟 ASR，但不能绑定单一厂商 | 本地演示先用 `mock` Provider 跑通闭环，真实环境再接入讯飞、阿里云、腾讯云、OpenAI、本地 ASR |
...
- 本地演示 Provider：`mock`，用于在没有云密钥时验证端到端链路。
