...
| ASR 能力 | 中文实时语音识别应优先选成熟 ASR，但不能绑定单一厂商 | 24 小时版先实现讯飞 Provider，同时预留阿里云、腾讯云、OpenAI、本地 ASR 的统一接口 |
...
- Web/PWA 输入法工作台，可部署到 Vercel、静态站点或任意前端托管。
- qiniu-ime XGo 后端服务：提供 ASR Gateway、用户记忆、热词纠错、评测报告。
- ASR Provider 插件化：默认接入讯飞，同时代码结构支持替换为阿里云、腾讯云、OpenAI、本地 FunASR/whisper.cpp。
...
- 默认 Provider：讯飞实时语音听写/转写，用于快速打通中文流式识别。
...
基础识别交给可替换 ASR Provider。24 小时版先接入讯飞 Provider，但业务层只依赖统一 `ASRProvider` 接口；后续接入阿里云、腾讯云、OpenAI、本地 ASR 时，不改输入法核心逻辑。
...
| 时间 | 任务 |
|---|---|
| 0-2h | 建 Next.js 前端、XGo 后端骨架、Dockerfile、演示文案 |
| 2-5h | 麦克风采集、PCM 转换、前端连接 qiniu-ime WebSocket |
| 5-8h | 后端 ASR Gateway + `ASRProvider` 接口 + 讯飞 Provider，转发 partial/final |
